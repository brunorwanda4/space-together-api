...
async fn get_config_by_subject_id(
    ...
    match service.get_config_by_main_subject_id(&subject_id).await {
        Ok(config) => HttpResponse::Ok().json(config),
        Err(message) => HttpResponse::NotFound().json(ReqErrModel { message }),
    }
}
...
async fn create_default_config(
    ...
    let creator_id = match ObjectId::parse_str(&logged_user.id) {
        Ok(oid) => Some(oid),
        Err(_) => {
    ...
    match service
        .get_or_create_default_config(&subject_id, creator_id)
        .await
    {
        Ok(config) => HttpResponse::Created().json(config),
        Err(message) => HttpResponse::BadRequest().json(ReqErrModel { message }),
...
pub fn init(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/subject-progress-configs")
            ...
            .service(create_default_config) // POST /subject-progress-configs/subject/{subject_id}/default
